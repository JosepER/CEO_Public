...
first_language_province %>%
  write_csv(here("interim_outputs", "calibration", "first_language_calibration_proportions_04.csv"))

place_of_birth %>%
  write_csv(here("interim_outputs", "calibration", "place_of_birth_proportions_04.csv"))
...
bind_rows(comparison_proportions_place_of_birth, comparison_proportions_first_language) %>%
  ...
  write_csv(here("interim_outputs", "calibration", "comparison_survey_863_population_calibration_proportions_04.csv"))
...
if(!file.exists(here("interim_outputs", "calibration", "summary_main_survey_weights_04.csv"))){

summary_main_survey_weights %>%
  gather(key = "indicator") %>%
  write_csv(here("interim_outputs", "calibration", "summary_main_survey_weights_04.csv"))
...
if(!file.exists(here("interim_outputs", "resample_survey_desings_863_bcn", "survey_designs_04.rds"))){

resamples_survey_design_list <- resamples_for_calibration %>%
  map(~ svydesign(ids = ~ 0, data = .x))

resamples_survey_design_list %>%
  write_rds(here("interim_outputs", "resample_survey_desings_863_bcn", "survey_designs_04.rds"))

}else{
  resamples_survey_design_list <- read_rds(here("interim_outputs", "resample_survey_desings_863_bcn", "survey_designs_04.rds"))
}
...
raked_resamples_list %>%
  write_rds(here("interim_outputs", "calibration", "raked_sampling_designs_resamples_04.rds"))

resamples_calibration_weights <- raked_resamples_list %>%
  map(~ weights(.x))

resamples_calibration_weights %>%
  write_rds(here("interim_outputs", "calibration", "raked_calibration_weights_resamples_04.rds"))
...
resamples_calibration_weights %>% 
  write_rds(here("interim_outputs", "calibration", "weights_resamples_untrimmed_04.rds"))

## first trimmed weights

resamples_calibration_weights_trimmed %>%
  write_rds(here("interim_outputs", "calibration", "weights_resamples_trimmed_1_04.rds"))

## final trimmed weights

resamples_calibration_weights_trimmed_2 %>%
  write_rds(here("interim_outputs", "calibration", "weights_resamples_trimmed_2_04.rds"))
