...
t_2 <- read_rds(here("outputs", "data_graph_comparison_bootstrap_designs_06.rds"))


p_1
```
_Note: See section ['Estimates of turnout in 1-O referendum' below](# Estimates of turnout in 1-O referendum) for details on the computation of these estimates._
...
1. Set quota targets for the quota variables of the survey. Quota variables, unlike stratums, don´t need to form mutually exclusive categories[^12];
...
3. Drop those responses from the first iteration resample that overfill a quota category, retain the rest[^13]. 
...
2. First language of respondent[^15].  

Calibration for each resample as well as for the main survey was implemented with a raking algorithm. This procedure is different from that used by CEO when computing their survey weights. They use a post-stratification calibration with crossed categories age, place of birth and first language[^16]. I didn't repeat their procedure for two resasons: 
...
* Bootstrap bias-corrected and accelerated interval (BCa): It is also based on percentiles from the bootstrap distribution of estimates, but these might not be the same as in the 'standard percentile interval' method. The percentiles depend on the 'acceleration' and 'bias-correction'. These changes are meant to correct deficiencies of the other bootstrap methods. This correction uses both the bootstrap and the jackknife resample estimates.
...
[^12]: For example, we can have a sample which resembles the population by categories in quota variable A and quota variable B but not for crossed categories of A and B.

[^13]: For example, if the quota target was 10 males 18-24 and the resample included 13 of these, drop the last 3.

ORDER FROM HERE:

[^14]: This would be something similar to the bootstrap for Stratified or Multistage Sampling explained in pages 207 to 213 of Wolter, K. M. (2007) Introduction to Variance Estimation, 2nd edn. New York: Springer.

[^15]: The language that respondents first used when they were children.

[^16]: See the table at the end of the technical report of the survey (In Catalan): http://upceo.ceo.gencat.cat/wsceop/6408/Fitxa%20t%C3%A8cnica%20-863.pdf

[^17]: I trimmed the top 0.5% of weights for all samples and limited the ratio between the minimum and maximum weight in each resample to be equal to 8. The code for this can be found in [script 03_calibrate_bootstrap_resamples.R](https://github.com/JosepER/CEO_public/blob/master/scripts/03_calibrate_bootstrap_resamples.R)